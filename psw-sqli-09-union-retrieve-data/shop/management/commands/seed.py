"""
Seed the UNION-retrieve-other-tables lab.

Administrator password is a random 16-byte token so the only way to
obtain it is via the intended UNION extraction. Low-privilege users
have memorable passwords so the attacker can verify the login view
...
FLAG_CONTENT = "FLAG{psw-sqli-09-union-retrieve-admin-access}"
