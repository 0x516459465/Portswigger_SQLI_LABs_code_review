"""
Views for the "list database contents on non-Oracle" SQLi lab.

Two surfaces:

1. `filter_products` is the vulnerable entry point: the `category`
   query parameter is concatenated straight into raw SQL. This is the
   pivot used to enumerate `information_schema` and extract admin
   credentials.
...
bound to an administrator account, so the full chain is:
    ...
    /filter?category=...   \u2192 UNION-inject information_schema
    \u2192 read users.username/password
