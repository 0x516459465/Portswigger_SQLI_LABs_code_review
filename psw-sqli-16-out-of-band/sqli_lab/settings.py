"""
Django settings for the "Blind SQLi with out-of-band interaction" lab.

PostgreSQL is the backend. The lab stands up three containers on a
private Docker network: db (Postgres + dblink), oob (TCP+HTTP
recorder acting as the stand-in for Burp Collaborator), and web
(this Django app). Only a SQL payload that coerces the db container
into an outbound connection to the oob container will get logged,
which is the signal the home view watches to release the flag.
"""
